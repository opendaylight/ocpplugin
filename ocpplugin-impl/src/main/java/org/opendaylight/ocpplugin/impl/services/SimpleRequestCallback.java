...
final class SimpleRequestCallback<T extends DataObject> extends AbstractRequestCallback<T> {
    ...
    private SimpleRequestCallback(final RequestContext<T> context, final Class<?> requestType, final Class<T> clazz) {
        super(context, requestType);
        this.clazz = Preconditions.checkNotNull(clazz);
    }

    static <T extends DataObject> FutureCallback<OcpHeader> create(final RequestContext<T> context, final Class<?> requestType, final Class<T> clazz) {
        return new SimpleRequestCallback<>(context, requestType, clazz);
    }

    @Override
    public final void onSuccess(final OcpHeader result) {
